...
class MoneyAggregator
{
    public function getMonthly(User $user, string $date)
    {
        ...
        $money = DB::table('sub_categories')
            ...
            ->select('money_flows.id', 'money_flows.user_id', 'money_flows.amount', 'money_flows.created_at',
                'sub_categories.name AS sub_category_name', 'money_flows.description', 'categories.name AS category_name',
